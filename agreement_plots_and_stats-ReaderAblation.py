...
comps = [x for x in comps if 'Only' in x]
comps = [x for x in comps if 'Bbox' not in x]
    
if not os.path.exists(os.path.join(sdir,'master_ablation_df.csv')):
    master_df = pd.DataFrame()
    for comp in comps:
        mr_dfs = os.listdir(os.path.join(mdir,comp))
        if comp=='HumansOnly-NoE':
    ...
    master_df.to_csv(os.path.join(sdir,'master_ablation_df.csv'))
else:
    master_df = pd.read_csv(os.path.join(sdir,'master_ablation_df.csv'))
...
plt.xticks(ticks=[0,1,2,3,4,5],rotation=0,labels=['All Readers','Excl. R1',
                                                   'Excl. R2','Excl. R3',
                                                   'Excl. R4','Excl. R5'])
plt.ylim([0,1])
plt.legend(bbox_to_anchor=(1.17,1.05))
plt.tight_layout()
plt.savefig(os.path.join(pdir,'HumansOnly_JI_ReaderAblation_MR.png'))
...
plt.tight_layout()
plt.savefig(os.path.join(pdir,'HumansOnly_SI_ReaderAblation_MR.png'))
...
for comp in comps:
    ...
    for th in [0.25,0.5,0.75]:
        ...
        s1 = sdf[sdf['Comparison']=='HumansOnly']
...
plt.legend(bbox_to_anchor=(1.0,1.05))
plt.tight_layout()
plt.savefig(os.path.join(pdir,'HumansOnly_JI_ReaderAblation_bySample.png'))
